...
const Table = ({ blogs, showMore, blogSetChanger, showMoreSetChanger }) => {
  ...
  // show the left of blog post by clicking on showmore button
  const handleShowMore = async () => {
    const startIndex = blogs.length;
    try {
      const updatedData = await handleGETData(
        `/blog/all-blogs?userId=${currentUser.data._id}&startIndex=${startIndex}`
      );
      if (updatedData.success == true) {
        blogSetChanger((prev) => [...prev, ...updatedData.blogs]);
        if (updatedData.blogs.length < 8) {
          showMoreSetChanger(false);
        }
      }
    } catch (error) {
  ...
  const handleDelete = async (id) => {
    const checked = confirm("Are you sure you want to delete the blog?");
    if (checked) {
      const result = await handleDeleteData(`/blog/delete/${id}`);
      if (result.success == true) {
        //through this we are filtering out the deleted blog post out of posts array
        blogSetChanger((prev) => prev.filter((blog) => blog._id !== id));
  ...
  return blogs.length >= 1 ? (
    <>
                      ...
                      >
                        ...
                        <td className=" p-4 ">
                          <div className="flex items-center gap-1">
                            <button
                              className="p-2 rounded-full group transition-all duration-500 text-violet-500 hover:underline"
                            ...
                            <button
                              ...
                              onClick={() =>
                                navigate(
                                  `/dashboard?tab=edit-blog&blog_id=${blog._id}`
                                )
                              }
                            >
                              <svg
      ...
      {showMore && (
        <button
          className="w-full text-teal-500 self-center text-sm py-7 font-semibold transition-all duration-500 hover:text-theme"
